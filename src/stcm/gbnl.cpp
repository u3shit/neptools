...
{
    return field_08 == 0xffffffff && field_18 == 0 && text_offset < chunk_size;
...
bool GbnlFooter::IsValid(size_t chunk_size) const noexcept
{
    return memcmp(magic, "GBNL", 4) == 0 &&
        ...
        field_28 == 0x0c &&
        offset_msgs + sizeof(GbnlMessageDescriptor) * count_msgs < chunk_size &&
        field_30 == 4 && field_34 == 0 && field_38 == 0 && field_3c == 0;
}
    ...
    : Item{k, ctx}
{
    ...
    for (size_t i = 0; i < foot->count_msgs; ++i)
    {
        ...
        messages.push_back({
            msgs[i].message_id, msgs[i].lipsync_chara_id, msgs[i].field_0c,
            msgs[i].field_10, msgs[i].name_id, msgs[i].audio_id,
...
void GbnlItem::Dump(std::ostream& os) const
{
    ...
    auto control_end = msgs_end + sizeof(GbnlControlDescriptor) * control.size();
...
void GbnlItem::PrettyPrint(std::ostream& os) const
{
    ...
    for (const auto& m : messages)
        os << "  (" << m.message_id << ", " << m.lipsync_chara_id << ", "
           << m.field_0c << ", " << m.field_10 << ", " << m.name_id << ", "
           << m.audio_id << ", " << m.text << ")\n";
    os << "] control[";
    for (const auto& c : control)
        os << "(" << c.first << ", " << c.second << ")";
...
size_t GbnlItem::GetSize() const noexcept
{
    size_t ret = sizeof(GbnlMessageDescriptor) * messages.size() +
        sizeof(GbnlControlDescriptor) * control.size();
